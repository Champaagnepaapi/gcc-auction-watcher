"""Opt-in PokemonPriceTracker observer for the final V4 external-market pipeline.

Safe-off by default. When enabled it performs bounded GET-only PPT requests,
records a shadow comparison, then returns the canonical V4 opportunities
unchanged. It never notifies and never changes fair value/max_recommended.
"""
...
PPT_BASE_URL = "https://www.pokemonpricetracker.com/api/v2/cards"
SHADOW_STATE_KEY = "v4_ppt_shadow"
SHADOW_SCHEMA_VERSION = 1
DEFAULT_MAX_HTTP_CALLS_PER_RUN = 12
DEFAULT_CREDIT_CAP_PER_RUN = 60
DEFAULT_DAILY_REMAINING_FLOOR = 15_000
DEFAULT_CACHE_TTL_HOURS = 6
DEFAULT_REQUEST_INTERVAL_SECONDS = 1.10
DEFAULT_TIMEOUT_SECONDS = 20.0
DEFAULT_MAX_STATE_RECORDS = 250
...
class PptRequestBudget:
    max_http_calls: int = DEFAULT_MAX_HTTP_CALLS_PER_RUN
    credit_cap: int = DEFAULT_CREDIT_CAP_PER_RUN
    daily_remaining_floor: int = DEFAULT_DAILY_REMAINING_FLOOR
    ...
    def wait(self) -> None:
        if self.last_call_monotonic is None or self.interval_seconds <= 0:
            return
        delay = self.interval_seconds - (time.monotonic() - self.last_call_monotonic)
        if delay > 0:
            time.sleep(delay)
...
def _grade_key(grader: object, grade: object) -> str:
    return f"{_norm(grader).replace(' ', '')}{_grade_text(grade).replace('.', '_')}"


def match_macro_identity(identity: PptMacroIdentity, rows: Sequence[Mapping[str, object]]) -> PptMacroMatch:
    by_external = [row for row in rows if _norm(row.get("externalCatalogId")) == _norm(identity.card_id)]
    if len(by_external) == 1:
        return PptMacroMatch("EXACT", by_external[0], "EXTERNAL_CATALOG_ID", 1)
    if len(by_external) > 1:
        return PptMacroMatch("AMBIGUOUS", None, "EXTERNAL_CATALOG_ID", len(by_external))
    by_set_number = [
        row for row in rows
        if _collector_numerator(row.get("cardNumber") or row.get("number")) == _collector_numerator(identity.number)
        and _set_compatible(identity.set_name, row.get("setName") or row.get("set_name"))
    ]
    if len(by_set_number) == 1:
        return PptMacroMatch("EXACT", by_set_number[0], "SET_NUMBER", 1)
    if len(by_set_number) > 1:
        return PptMacroMatch("AMBIGUOUS", None, "SET_NUMBER", len(by_set_number))
...
def _graded(row: Mapping[str, object], grader: str, grade: object) -> GradedAggregate | None:
...
def _search_attempts(identity: PptMacroIdentity) -> tuple[dict[str, object], ...]:
    return (
        {"search": identity.name, "setName": identity.set_name, "limit": 5},
        {"search": f"{identity.name} {_collector_numerator(identity.number)}", "setName": identity.set_name, "limit": 5},
        {"search": identity.name, "limit": 10},
    )


def _get(session: requests.Session, key: str, budget: PptRequestBudget, params: Mapping[str, object], timeout: float) -> tuple[int | None, object | None]:
    if not budget.can_call():
        return None, None
    budget.wait()
    try:
        response = session.get(PPT_BASE_URL, headers={"Authorization": f"Bearer {key}", "Accept": "application/json"}, params=dict(params), timeout=timeout)
...
def fetch_snapshot(identity: PptMacroIdentity, grader: str, grade: object, key: str, budget: PptRequestBudget, session: requests.Session, timeout: float) -> tuple[str, GradedAggregate | None, list[DailyGradePoint], str]:
    ...
    proof = ""
    for params in _search_attempts(identity):
        status, payload = _get(session, key, budget, params, timeout)
        if status is None:
    ...
    status, payload = _get(session, key, budget, {
    ...
    if deep.status != "EXACT" or deep.row is None:
        return deep.status, None, [], f"DEEP_{deep.proof or 'NO_MATCH'}"
    aggregate = _graded(deep.row, grader, grade)
...
def _root(state: dict) -> dict[str, Any]:
    root = state.get(SHADOW_STATE_KEY)
    if not isinstance(root, dict) or root.get("schema_version") != SHADOW_SCHEMA_VERSION:
        root = {"schema_version": SHADOW_SCHEMA_VERSION, "cache": {}, "records": {}}
        state[SHADOW_STATE_KEY] = root
    root.setdefault("cache", {})
    root.setdefault("records", {})
    return root


def _cache_get(root: dict[str, Any], identity_key: str, now: datetime, ttl_hours: int) -> dict[str, Any] | None:
    item = root.get("cache", {}).get(identity_key)
    if not isinstance(item, Mapping):
        return None
    fetched = _aware(item.get("fetched_at"))
    metrics = item.get("metrics")
    if fetched is None or now - fetched > timedelta(hours=ttl_hours) or not isinstance(metrics, Mapping):
        return None
    return dict(metrics)
...
def _gcc_exact_count(candidate: Any) -> int:
    ...
    for sale in getattr(candidate.gcc, "sales", []) or []:
        if getattr(sale, "exact_card", True) is False or str(getattr(sale, "grader", "") or "").strip().upper() != target_grader:
            continue
        try:
            sale_grade = float(getattr(sale, "grade", None))
        except (TypeError, ValueError):
            continue
        if target_grade is not None and sale_grade == float(target_grade):
            count += 1
    return count


def _current(candidate: Any, opportunity_by_key: Mapping[str, Any]) -> dict[str, Any]:
    import watcher
    key = watcher.external_commercial_identity_key(candidate.lot)
...
def _effect(metrics: Mapping[str, Any], current: Mapping[str, Any]) -> str:
    if metrics.get("evidence_strength") != "STRONG" or not metrics.get("kinetic_shadow_signal"):
        return "NO_SHADOW_SIGNAL"
    if current.get("gcc_branch") != "SUPPORTED":
        return "PPT_EXTERNAL_RESCUE_CANDIDATE"
    if not current.get("already_opportunity"):
        return "PPT_REVALUE_CANDIDATE"
    return "PPT_SUPPORTS_OR_REPRICES_CURRENT_OPPORTUNITY"


def _record(root: dict[str, Any], identity_key: str, record: Mapping[str, Any]) -> None:
    records = root["records"]
    records[identity_key] = dict(record)
    maximum = _env_int("V4_PPT_SHADOW_MAX_STATE_RECORDS", DEFAULT_MAX_STATE_RECORDS, 10)
...
def collect_ppt_shadow(candidates: Sequence[Any], opportunities: Sequence[Any], state: dict, now: datetime, *, session: requests.Session | None = None) -> dict[str, int]:
    """Observe all exact candidates, including candidates that already have GCC SOLD."""
    import watcher
    import v4_canonical_multimarket as canonical_market

    summary = {"eligible": 0, "matched": 0, "strong": 0, "cache_hits": 0, "blocked_variant": 0, "rescue_candidates": 0, "revalue_candidates": 0}
    key = os.getenv("POKEMONPRICETRACKER_API_KEY", "").strip()
    if not key:
        return summary
    root = _root(state)
    budget = PptRequestBudget(
        max_http_calls=_env_int("V4_PPT_SHADOW_MAX_HTTP_CALLS_PER_RUN", DEFAULT_MAX_HTTP_CALLS_PER_RUN),
        credit_cap=_env_int("V4_PPT_SHADOW_CREDIT_CAP_PER_RUN", DEFAULT_CREDIT_CAP_PER_RUN),
        daily_remaining_floor=_env_int("V4_PPT_SHADOW_DAILY_REMAINING_FLOOR", DEFAULT_DAILY_REMAINING_FLOOR),
        interval_seconds=_env_float("V4_PPT_SHADOW_REQUEST_INTERVAL_SECONDS", DEFAULT_REQUEST_INTERVAL_SECONDS),
    )
    timeout = _env_float("V4_PPT_SHADOW_TIMEOUT_SECONDS", DEFAULT_TIMEOUT_SECONDS, 1.0)
    ttl = _env_int("V4_PPT_SHADOW_CACHE_TTL_HOURS", DEFAULT_CACHE_TTL_HOURS, 1)
    session = session or requests.Session()
    opportunities_by_key = {watcher.external_commercial_identity_key(op.lot): op for op in opportunities}
    ...
    for candidate in candidates:
        ...
        if canonical.status != "EXACT":
            continue
        _, deterministic_variant = canonical_market._raw_variant_choice(lot, canonical)
        ...
        if target_grade is None or not grader or lot.current_price is None:
            continue
        identity_key = watcher.external_commercial_identity_key(lot)
        current = _current(candidate, opportunities_by_key)
        summary["eligible"] += 1
        metrics = _cache_get(root, identity_key, now, ttl)
        if metrics is not None:
            ...
            status, aggregate, history, proof = fetch_snapshot(
                ...
                grader, target_grade, key, budget, session, timeout,
            )
            if status != "MATCHED" or aggregate is None:
                _record(root, identity_key, {"observed_at": now.isoformat(), "status": status, "reason": proof, "evidence_class": EVIDENCE_CLASS, "upstream_class": UPSTREAM_CLASS, "current_v4": current})
                continue
            summary["matched"] += 1
            metrics = asdict(analyze_shadow(
                ShadowInput(True, True, grader, _grade_text(target_grade), float(lot.current_price), usd_per_eur, int(current["gcc_exact_sold_count"] or 0)),
        ...
        fair_eur = _positive(metrics.get("fair_value_eur"))
        metrics = dict(metrics)
        if fair_eur is not None:
            discount = (fair_eur - float(lot.current_price)) / fair_eur * 100.0
            ...
            required = float(metrics.get("shadow_required_discount_pct") or BASE_REQUIRED_DISCOUNT_PCT)
            metrics["kinetic_shadow_signal"] = bool(metrics.get("evidence_strength") == "STRONG" and discount >= required)
        if metrics.get("evidence_strength") == "STRONG":
            summary["strong"] += 1
        effect = _effect(metrics, current)
        if effect == "PPT_EXTERNAL_RESCUE_CANDIDATE":
            summary["rescue_candidates"] += 1
        elif effect == "PPT_REVALUE_CANDIDATE":
        ...
        _record(root, identity_key, {
            ...
            "grader": grader, "grade": _grade_text(target_grade), "gcc_price_eur": lot.current_price,
            "current_v4": current, "ppt_shadow": metrics, "shadow_effect": effect,
            "production_economic_use": False, "notification_use": False,
        })

    root["last_run"] = {
        "observed_at": now.isoformat(), "summary": summary, "http_calls": budget.http_calls,
        "credits": budget.credits, "daily_remaining": budget.daily_remaining,
        "blocked_reason": budget.blocked_reason, "production_economic_use": False,
    }
    return summary


_ORIGINAL_PROCESS_EXTERNAL = None


def install_v4_ppt_shadow() -> None:
    global _ORIGINAL_PROCESS_EXTERNAL
    import watcher
    ...
    def wrapped(page, candidates, state, budgets, run_diagnostics, now, *args, **kwargs):
        opportunities = _ORIGINAL_PROCESS_EXTERNAL(page, candidates, state, budgets, run_diagnostics, now, *args, **kwargs)
        try:
    ...
    watcher.log("PPT shadow: enabled (SOLD_AGGREGATED only; no FV/max/notification changes)")

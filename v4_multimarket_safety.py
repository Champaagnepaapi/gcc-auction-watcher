...
def _provider_name_matches(canonical: mm.CanonicalCard, candidate_name: object) -> bool:
    ...
    # Post-#127 live diagnostics returned exact pokemon-japanese candidates with
    # a literal trailing `(Japanese)` display annotation, e.g. `Galarian Zapdos
    # (Japanese)`. Accept only that exact suffix, only for an exact JA canonical
    # card, and only when the remaining base name equals the canonical name.
    japanese = re.fullmatch(r"(.+?)\s*\(Japanese\)\s*", raw, flags=re.IGNORECASE)
    if japanese and str(canonical.language_code or "").strip().casefold() in {"ja", "jp"}:
        if mm._normalize(japanese.group(1)) == mm._normalize(canonical.name):
            return True

    # PokeTrace appends `(Secret)` to some cards whose printed collector number
    # itself proves the secret-rarity relation (numerator > denominator). Strip
    # only this exact trailing provider label; it cannot manufacture a finish,
    # edition, stamp or other sensitive microvariant.
    secret = re.fullmatch(r"(.+?)\s*\(Secret\)\s*", raw, flags=re.IGNORECASE)
    if secret and _numeric_secret_number(canonical.full_number):
        return mm._normalize(secret.group(1)) == mm._normalize(canonical.name)
    return False
...
def hardened_candidate_exact_for_canonical(
    ...
    variants = canonical.variants if isinstance(canonical.variants, Mapping) else {}
    # `firstEdition: true` means material edition differentiation is applicable,
    # not that this particular listing is First Edition. Without a listing
    # edition signal, automatic graded economics must stop here.

...
class PokeTraceProviderBridgeTests(unittest.TestCase):
    ...
    def test_japanese_display_suffix_is_bounded_to_exact_ja_base_name(self):
        japanese = _canonical(
            card_id="S12a-188",
            set_id="S12a",
        ...
        self.assertTrue(
            safety._provider_name_matches(japanese, "Galarian Zapdos (Japanese)")
        )
        self.assertFalse(
            safety._provider_name_matches(english, "Galarian Zapdos (Japanese)")
        )
        self.assertFalse(
            safety._provider_name_matches(japanese, "Galarian Moltres (Japanese)")
        )
    ...
    def test_post_127_galarian_zapdos_live_shape_passes_exact_bridges(self):
        target = _lot(
            name="Galarian Zapdos",
            reference="188/172",
            language="Japanese",
        ...
        candidate["set"] = {"name": "S12: Silver Tempest"}
        self.assertFalse(
            safety.hardened_candidate_exact_for_canonical(target, canonical, candidate)
        )

"""Recover exact Magi set-name cards when only rarity separates same-name prints.

This Global-only fallback runs only after the existing exact-set/name resolver
has proved that one exact Japanese TCGdex set is present but multiple cards with
the same Japanese name remain possible. It never guesses a set or card name.

Current reviewed provider normalization is deliberately narrow:
- standalone Magi token ``R`` -> TCGdex rarity ``Rare``.

Any other rarity token, missing rarity, multiple distinct rarity tokens, TCGdex
transport error, or more than one surviving card remains blocked. The exact
selected card is revalidated by card ID, localId, Japanese name, set ID/name,
official denominator and rarity before a CommercialIdentity is produced.
"""
...
_REVIEWED_RARITY_MAP = {"R": "Rare"}
_ORIGINAL_RESOLVER = None
_INSTALLED = False


def _provider_rarity(title: str) -> tuple[str, str]:
    tokens = {match.group(1).upper() for match in _RARITY_TOKEN_RE.finditer(str(title or ""))}
    if len(tokens) != 1:
...
def recover_set_name_rarity_unique_card_resolution(
    ...
    *,
    ...
    return native.MagiNativeResolution(
        "EXACT",
        f"MAGI_NATIVE_TCGDEX_JA_EXACT_SET_NAME_RARITY_UNIQUE_CARD+{rarity_reason}",
        identity=identity,
        card_id=proof.card_id,
        set_id=proof.set_id,
    )

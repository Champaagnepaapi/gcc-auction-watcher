...
État réel de ce batch : **REAL NEON VALIDATION MIGRATION PASS** sur la seule branche `p3-migration-validation` (`br-noisy-grass-axr6inqe`) de `robot-pokemon-kb`, base `neondb`. La correction ciblée est le commit `e898c280dd30d585e23c99f241277fdca802713e`. Le ledger réel conserve `0001_durable_shadow.sql` au checksum original `c5357dc1dcfa99121c993c4d4567aae886990bf52ddcfb7ca93fe9266c04dffd` et ajoute `0002_trigger_alias_safety.sql` (`9e7cb1d05ec6be333267434109bb07f0ff5dad73a70863b22eb858ed5f45599e`). La première migration du pilote a inséré 1 099 lignes ; la seconde en a inséré 0 et a reproduit exactement les mêmes counts et fingerprints.

Les 34/34 payloads ont été comparés entre SQLite et Neon par octets, longueur et SHA-256 : 31 GCC et 3 TCGdex, tous identiques. Les contrôles réels donnent 0 fait orphelin, 0 foreign key invalide, 0 observation non scellée, 0 trigger public désactivé, une contrainte unique d’idempotence active, 0 `SALE_TRANSACTION` au total et pour TCGdex, et 45 `PROVIDER_METRIC_OBSERVATION`. Le replay offline rollback-only des 34 payloads a laissé les tables économiques/retrieval strictement inchangées ; il a rejoué 76 observations, sans vente dupliquée, puis annulé volontairement les deltas non économiques prospectifs (93 claims, 31 résolutions et 31 liens). Neon `main` n’a reçu aucune mutation et conserve `written_data_bytes = 0`. Baseline SQLite et résultat Neon mesurés, jamais hard-codés par l’outil :
...
Pour vérifier une restauration, créer d’abord une base/branche Neon temporaire vide, définir sa URL dans `ROBOT_KB_RESTORE_DATABASE_URL`, puis lancer `python -m robot_kb.postgres_backup restore-verify /chemin/backup.dump`. L’outil restaure avec `pg_restore --exit-on-error`, puis compare les counts et fingerprints de toutes les tables. Supprimer uniquement la base/branche temporaire après succès. Aucun dump/restore réel n’a été effectué dans ce batch de validation ciblée ; cette opération reste séparée de la migration pilote demandée.
...
**AUTO SCHEDULER NOT ENABLED.** Il n’existe aucun `schedule:` dans ce workflow et aucun dispatch n’a été lancé. La migration pilote est validée uniquement sur la branche Neon de validation ; toute promotion vers Neon `main`, tout cron cloud, dump/restore réel ou collecte manuelle exige encore une approbation Red Team ciblée. V4 et ses workflows de production sont inchangés. V5 / PR #8 reste draft, non touchée et non mergée.

...
class VerifiedOfferAdapterTests(unittest.TestCase):
    def test_all_target_markets_can_feed_fixed_asks(self):
        for market in ("gcc", "cardova", "magi", "fanatics", "comc"):
            row = verified_fixed_ask(market=market, identity=ID, price=100, currency="EUR", observed_at=NOW)
    ...
    def test_unknown_market_rejected(self):
        with self.assertRaises(ValueError):
            verified_fixed_ask(market="unknown", identity=ID, price=100, currency="EUR", observed_at=NOW)

    def test_auction_state_is_explicit(self):
        active = verified_auction_snapshot(market="fanatics", identity=ID, price=50, currency="USD", observed_at=NOW, end_at=NOW, within_five_minutes=False, buyer_fee_rate=0.20)
        final = verified_auction_snapshot(market="fanatics", identity=ID, price=50, currency="USD", observed_at=NOW, end_at=NOW, within_five_minutes=True, buyer_fee_rate=0.20)
